...
def handleSelection(core, owner, target, option):
    global ngeCore
    ngeCore = core.getInstance()
    suiSvc = ngeCore.suiService
    
    # Create Guild
    if option == 185 and target:
        wndGuildCreate = suiSvc.createInputBox(InputBoxType.INPUT_BOX_OK_CANCEL, '@guild:create_name_title', '@guild:create_name_prompt', owner, owner, 0, handleGuildCreateName)
...
def handleGuildCreateName(owner, window, eventType, returnList):
    ...
    if eventType == 0:
        ...
        if guildName is not None:
        	...
        	wndGuildInitials = suiSvc.createInputBox(InputBoxType.INPUT_BOX_OK, '@guild:create_abbrev_title', '@guild:create_abbrev_prompt', owner, owner, 0, doGuildCreate)
...
def doGuildCreate(owner, window, eventType, returnList):
    if eventType == 0:
        guild = ngeCore.guildService.createGuild(str(returnList.get(0)), str(owner.getAttachment('guildName')), owner)
